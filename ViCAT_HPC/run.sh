...
JOB_ID=`sbatch $ARGS --export=ALL,BAMTOOLS=$BAMTOOLS,SAMPLE_DIR=$SAMPLE_DIR,ALLCONSENSUS=$ALLCONSENSUS,OUT_DIR=$OUT_DIR,STDERR_DIR6=$STDERR_DIR6,STDOUT_DIR6=$STDOUT_DIR6,PROFILE=$PROFILE,ALLBLAST=$ALLBLAST,BOWTIE=$BOWTIE,BOWTIEBUILD=$BOWTIEBUILD,ERRORS=$ERRORS --job-name $Prog6 --dependency=afterok:$PREV_JOB_ID -o $STDOUT_DIR6/output.%a.out -e $STDERR_DIR6/err.%a.out -a 1-$NUM_JOB $SCRIPT_DIR/run_bowtie2.sh`
